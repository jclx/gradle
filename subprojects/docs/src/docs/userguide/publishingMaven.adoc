...
You can configure the tasks generated by the “`maven-publish`” plugin to be skipped based on certain criteria. The following sample demonstrates how to publish the `binary` and `binaryAndSources` publications only to the `external` and `internal` repositories, respectively. In addition, it configures only `binaryAndSources` to be published to the local Maven repository.
...
++++
<sample dir="maven-publish/conditional-publishing" id="shorthandTasks" title="Adding multiple publications and repositories">
